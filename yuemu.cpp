        ...
        switch (opcode_category) {
            case 0x0: { // load store
                switch (opcode_id) {
                    case 0x0: { // load immediate
                        ...
                        if (DEBUG_LEVEL >= 10) {
                            std::cout << "loadimm: rd=" << rd << ", val=" << to_signed(val) << "\n";
                        }
                        break;
                    }
                    ...
                    case 0x3: { // storemd 0000_0011_16-bits-addr_8-bits-rs
                        uint32_t addr = instr >> 8 & 0xFFFF; // 16 bits
                        uint32_t rs = instr & 0xFF; // 8 bits
                        mem[addr] = regs[rs];

                        if (DEBUG_LEVEL >= 10) {
                            std::cout << "storemd: addr=" << addr << ", rs=" << rs << "\n";
                            std::cout << "+> value_at_rs=" << to_signed(regs[rs]) << "\n";
                        }
                        break;
                    }
                    ...
                    case 0x4: { // load direct 0000_0100_8-bits-rd_16-bits-addr
                        ...
                        if (DEBUG_LEVEL >= 10) {
                            std::cout << "loaddir: rd=" << rd << ", raddr=" << addr << "\n";
            ...
            case 0x1: { // arithmetic
                switch (opcode_id) {
                    ...
                    case 0x3: { // less than or equal to
                        uint32_t rd = instr >> 16 & 0xFF;
                        uint32_t rs1 = instr >> 8 & 0xFF;
                        uint32_t rs2 = instr & 0xFF;
                    ...
                    case 0x5: { // greater than or equal to
                        uint32_t rd = instr >> 16 & 0xFF;
                        uint32_t rs1 = instr >> 8 & 0xFF;
                        uint32_t rs2 = instr & 0xFF;
            ...
            case 0x2: { // control
                switch (opcode_id) {
                    case 0x00: { // jump unconditionally immediate
                        uint32_t val = instr & 0xFFFFF; // 24 bits address value
                    ...
                    case 0x3: { // jump if direct 0010_0011_8-bits-rs_8-bits-skip_8-bits-rcond
                        uint32_t rs = instr >> 16 & 0xFF;
                        uint32_t rcond = instr & 0xFF;
